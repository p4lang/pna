...
multiple NIC devices that are conformant to the PNA [Footnote 1].

[Footnote 1] Of course, given the tight hardware resource
...
| packet from network port             1.3+^| main, with direction `NET_TO_HOST` 1.3+^|Zero or more mirrored packets, plus at most one of: a net-to-host recirculated packet, or one to-host packet.
| packet from net-to-host recirculate 
| packet from  port loopback
| packet from  message processing      1.3+^| main, with  direction `HOST_TO_NET` 1.3+^|Zero or more mirrored packets, plus at most one of: a host-to-net recirculated packet, or one to-net packet.
...
from one PNA implementation to another [Footnote 2].

[Footnote 2]: It is expected that `pna.p4` include files for
